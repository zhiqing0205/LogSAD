...
# from torchvision.transforms.v2.functional import to_tensor,to_image_tensor
...
def read_image(path: str | Path, as_tensor: bool = False) -> torch.Tensor | np.ndarray:
    ...
    # return to_tensor(to_image_tensor(image), torch.float32, scale=True) if as_tensor else np.array(image) / 255.0
