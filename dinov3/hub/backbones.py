...
def load_dinov3_model_layers(model_name, layers_to_extract_from, weight_type = None, pretrained_weight_path = None):
    assert model_name in MODELS, "model name must in {}".format(str(list(MODELS.keys())))
    model = MODELS[model_name](pretrained = False, weights= weight_type, layers_to_extract_from = layers_to_extract_from)
...
def load_dinov3_model(model_name, weight_type = None, pretrained_weight_path = None):
    assert model_name in MODELS, "model name must in {}".format(str(list(MODELS.keys())))
    model = MODELS[model_name](pretrained = False, weights= weight_type)
    if pretrained_weight_path is not None:
        state_dict = torch.load(pretrained_weight_path, map_location="cpu")
        res = model.load_state_dict(state_dict, strict=True)
        print(res)
...
def load_dinov3_model_dinomaly(model_name, layers_to_extract_from, weight_type = None, pretrained_weight_path = None):
    assert model_name in MODELS, "model name must in {}".format(str(list(MODELS.keys())))
    model = MODELS[model_name](pretrained = False, weights= weight_type, layers_to_extract_from = layers_to_extract_from)
    if pretrained_weight_path is not None:
        state_dict = torch.load(pretrained_weight_path, map_location="cpu")
